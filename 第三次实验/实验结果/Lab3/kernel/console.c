...
unsigned int lineFeed[(V_MEM_SIZE >> 1) / SCREEN_WIDTH][2]; //保存换行前后的cursor,0位表示上一行位置对应的下标，1位表示换行后当前行位置对应的下标
int lfIdx = -1;
...
PRIVATE int match_line_feed(unsigned int cursor)
{
	for (int i = 0; i <= lfIdx; i++)
	{
		if (lineFeed[i][0] == cursor)
		{
			return lineFeed[i][1];
		}
	}
...
/*======================================================================*
			   do_search
*======================================================================*/
PRIVATE void do_search(CONSOLE *p_con)
{
	if (searchTxtIdx == -1)
	{
		return;
    ...
    int search_start_cursor = start_cursor;
	u8 *search_p_vmem = p_vmem;
	...
	while (start_cursor < end_cursor)
	{
		...
		for (int i = 0; i <= searchTxtIdx; i++)
		{
			...
			if (searchTxt[i] == '\t')
			{
				...
				if (i == searchTxtIdx)
				{
					...
					{
						/*****************背景着色******************/
						if (temp_cursor - start_cursor >= 4)
						{
							if (match_tab(start_cursor + 4))
							{
						        	//search_start_cursor = temp_cursor;
						        	//search_p_vmem = temp_p_vmem;
						        	if(i==0){
						        		search_start_cursor += 3;
						        		search_p_vmem += 6;
						        	}
								for (int tabCount = 0; tabCount < 4; ++tabCount)
								{
									start_cursor++;
						            *(++p_vmem) = SEARCH_TAB_COLOR;
						            ++p_vmem;
								}
								continue;
							}
						}
						if (match_line_feed(start_cursor))
			            {
							int this_line_cursor = start_cursor;
							int next_line_cursor = match_line_feed(start_cursor);
							start_cursor = next_line_cursor;
							p_vmem  += 2*(next_line_cursor - this_line_cursor+1);
							search_start_cursor = next_line_cursor;
							search_p_vmem  += 2*(next_line_cursor - this_line_cursor+1);
							break;
						}
						if(*p_vmem == ' '){
						    start_cursor++;
						    *(++p_vmem) = SEARCH_BKG_COLOR;
						    ++p_vmem;
							continue;
						}
						/*******************************************/
				...
				if (i == searchTxtIdx)
				{
					...
					while (start_cursor < temp_cursor)
					{
						...
						if (match_line_feed(start_cursor))
			            {
							int this_line_cursor = start_cursor;
							int next_line_cursor = match_line_feed(start_cursor);
							start_cursor = next_line_cursor+1;
							p_vmem  += 2*(next_line_cursor - this_line_cursor+1);
							search_start_cursor = next_line_cursor+1;
							search_p_vmem  += 2*(next_line_cursor - this_line_cursor+1);
							break;
						}
						if(*(p_vmem) == ' '){
		...
		if(search_start_cursor != start_cursor){
			start_cursor = search_start_cursor;
			p_vmem = search_p_vmem;
		}
	}
}
...
PUBLIC void recover(TTY *p_tty)
{
	...
	/*
	for(int tabC=0;tabC<=tabIdx;++tabC){
	    out_char(p_tty->p_console, (char) tab[tabC]+64);
	}*/
}
			...
			p_con->original_addr + p_con->v_mem_limit - 1)
		{
			*p_vmem++ = ch;
			if (isSearch)
			{
				...
				if(ch == ' ') *p_vmem++ = SEARCH_BKG_COLOR;
				else *p_vmem++ = SEARCH_CHAR_COLOR;
			}
			else
			{
				*p_vmem++ = DEFAULT_CHAR_COLOR;
